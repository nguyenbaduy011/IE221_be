...
from authen.permissions import (
    IsAdminRole,
    IsAdminOrSupervisor,
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        return Response(
            {
                "status": "error",
                "message": "Hệ thống không cho phép xóa User. Vui lòng Deactivate.",
            },
    ...
    def deactivate(self, request, pk=None):
        ...
        if user.id == request.user.id:
            return Response(
                {"status": "error", "message": "Không thể tự khóa chính mình"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if request.user.role == "SUPERVISOR" and user.role == "ADMIN":
            return Response(
                {"status": "error", "message": "Không có quyền khóa Admin"},
        ...
        user.save()
        return Response(
            {"status": "success", "message": "Đã khóa tài khoản", "data": None},
    ...
    def activate(self, request, pk=None):
        ...
        if request.user.role == "SUPERVISOR" and user.role == "ADMIN":
            return Response(
                {"status": "error", "message": "Không có quyền mở khóa Admin"},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        user.save()
        return Response(
            {"status": "success", "message": "Đã mở khóa tài khoản", "data": None},
            status=status.HTTP_200_OK,
        )
    ...
    def bulk_deactivate(self, request):
        ...
        if not ids:
            return Response(
                {"status": "error", "message": "No user IDs provided"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def create(self, request, *args, **kwargs):
        if request.user.role == "SUPERVISOR":
            return Response(
                {"status": "error", "message": "Supervisor không có quyền tạo User"},
                status=status.HTTP_403_FORBIDDEN,
            )
    ...
    def update(self, request, *args, **kwargs):
        if request.user.role == "SUPERVISOR":
            return Response(
                {"status": "error", "message": "Supervisor không có quyền sửa User"},
                status=status.HTTP_403_FORBIDDEN,
            )
    ...
    def bulk_add(self, request):
        if request.user.role == "SUPERVISOR":
            return Response(
                {"status": "error", "message": "Supervisor không có quyền Bulk Add"},
                status=status.HTTP_403_FORBIDDEN,
            )
...
class UserTaskViewSet(
    ...
    def update(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": "Cập nhật task thành công", "data": serializer.data},
            status=status.HTTP_200_OK,
        )
...
class TraineeMySubjectsView(APIView):
    ...
    def get(self, request):
        ...
        my_subjects = (
            UserSubject.objects.filter(user=user)
            .select_related("course_subject", "course_subject__subject")
            .order_by("-created_at")
        )
